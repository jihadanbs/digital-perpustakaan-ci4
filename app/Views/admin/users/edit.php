                ...
                <div class="col-10">
                    <div class="card border border-secondary rounded p-4">
                        <div class="card-body">
                            ...
                            <?php if (is_object($tb_buku) || is_array($tb_buku)) : ?>
                                <form action="<?= esc(site_url('/admin/users/update/' . $tb_buku->id_buku), 'attr') ?>" method="post" enctype="multipart/form-data" id="validationForm" novalidate>
                                    ...
                                    <?php if (is_object($validation)) : ?>
                                        <div class="row">
                                            ...
                                            <div class="col-md-6 mb-3">
                                                ...
                                                <select class="form-select custom-border <?= ($validation->hasError('id_kategori_buku')) ? 'is-invalid' : ''; ?>" id="id_kategori_buku" name="id_kategori_buku" aria-label="Default select example" style="background-color: white;" required>
                                                    ...
                                                    <?php if (!empty($tb_kategori_buku) && (is_array($tb_kategori_buku) || is_object($tb_kategori_buku))) : ?>
                                                        <?php foreach ($tb_kategori_buku as $value) : ?>
                                                            <?php
                                                            $selected = ((string)$value->id_kategori_buku === (string)old('id_kategori_buku', $tb_buku->id_kategori_buku)) ? 'selected' : '';
                                                            ?>
                                                            <option value="<?= esc($value->id_kategori_buku, 'attr') ?>" <?= $selected ?>><?= esc($value->nama_kategori, 'html') ?></option>
                                                        <?php endforeach; ?>
                                                    <?php else : ?>
                                                        <option value="" disabled>Tidak ada data kategori buku</option>
                                                    <?php endif; ?>
                                                </select>
                                                <div class="invalid-feedback">
                                        ...
                                        <div class="form-group mb-4 mt-4">
                                            <div class="d-grid gap-2 d-md-flex justify-content-end">
                                                <a href="<?= esc(site_url('/admin/users/cek/' . $tb_buku->slug), 'attr') ?>" class="btn btn-secondary btn-md ml-3">
